...
const f2_1 = @import("f2_1_async_budget.zig");
    ...
    while (tick <= config.max_ticks) : (tick += 1) {
        ...
        if (!budget_exhausted) {
            ...
            while (node < config.world.population_size) : (node += 1) {
                ...
                states[node].sent.unionWithFacts(
                    action.facts,
        ...
        if (states[scaling.collector_index].knowledge.containsAll(
            ...
            result.success = true;
            if (budget_exhausted) {
                result.drain_ticks = tick - result.budget_exhausted_tick;
            }
            break;
        }

        if (!budget_exhausted and allBudgetConsumed(
            local_round,
            config.world.population_size,
            config.decision_budget_per_operator,
        )) {
            budget_exhausted = true;
            result.budget_exhausted_tick = tick;
        }
...
pub fn horizonForBudgetAndDrain(
    budget: u32,
    clock_jitter: u16,
    latency_min: u16,
    latency_jitter: u16,
) u32 {
    return horizonForBudget(budget, clock_jitter) +
        @as(u32, latency_min) +
        @as(u32, latency_jitter) +
        1;
}
...
test "F2.2 budget censoring consumes every operator budget" {
    var config = smokeConfig();
    ...
    if (!result.success) {
        ...
        try std.testing.expectEqual(@as(u64, 0), result.pending_envelopes);
        try std.testing.expect(result.budget_exhausted_tick > 0);
    }
}
