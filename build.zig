...
pub fn build(b: *std.Build) void {
    ...
    const f2_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/f2_test_root.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    const run_f2_tests = b.addRunArtifact(f2_tests);
    ...
    test_step.dependOn(&run_f2_tests.step);
    ...
    addRunStep(b, target, optimize, "run-f2", "Run F2 asynchrony/scaling experiments", "src/f2_run.zig");
}
